...
public class BloggerClient {
  ...
  public static Entry getLatestPost(BloggerService myService)  throws ServiceException, IOException {
	  URL feedUrl = new URL(feedUri + POSTS_FEED_URI_SUFFIX);
	    Feed resultFeed = myService.getFeed(feedUrl, Feed.class);
  ...
  public static void run(BloggerService myService, String userName,
    ...
    Entry recentPost=getLatestPost(myService);
    String plainText = recentPost.getTextContent().getContent().getPlainText();
    String[] split = plainText.split("-");

    String selfLinkHref = recentPost.getSelfLink().getHref();
    String[] tokens = selfLinkHref.split("/");
    String postId = tokens[tokens.length - 1];
    
    for (int i=1;i<split.length;i++) {
    	String string=split[i].trim();
    	// The below lines needed if anything is written after the stock name list
    	if(i==(split.length -1))
    		string=string.substring(0,string.indexOf(' '));
    	string="\""+string.trim()+"\"";
    	String searchTweet = TwitterSearch.searchTweet(string);
    	if(!searchTweet.isEmpty()){
    		System.out.println(searchTweet);
    		createComment(myService, postId, searchTweet);
    	}
    }
  ...
  public static void main(String[] args) {
    ...
    String userName =args[0];// parser.getValue("username", "user", "u");
    String userPassword = args[1];//parser.getValue("password", "pass", "p");
